...
psp_fmgr_getExtId(const char *szFilePath) 
{
  ...
  if((pszExt = strrchr(szFilePath, '.'))) {
    if (!strncmp(pszExt, ".9", 2)) {
      return FMGR_FORMAT_92;
...
psp_fmgr_ask_confirm(void)
{
  ...
  psp_sdl_back_print(190,  70, "Delete a file", 
                     PSP_MENU_WARNING_COLOR);
  psp_sdl_back_print(170,  80, "press B to confirm !", 
                     PSP_MENU_WARNING_COLOR);
  psp_sdl_flip();
...
static  int sel=0;
  ...
  for(;;) 
  {
    ...
    psp_sdl_back_rectangle(x, y, 190, rows * 10);
...
# if defined(WIZ_MODE)
    if ( c.Mouse && ((! danzeff_mode) || (! danzeff_isMouseInside( &c )))) {
      if ((c.My > 15) && (c.My < y)) {
        sel = top + (( c.My - 15 ) / 10);
        if (c.Mouse & GP2X_MOUSE_CLICK) new_pad = GP2X_CTRL_CROSS;
      } else 
      if (c.Mouse & GP2X_MOUSE_BTN_PRESS){
        if (c.My < 15) new_pad = GP2X_CTRL_UP;
        else
        if (c.My > y ) new_pad = GP2X_CTRL_DOWN;
      } else
      if (c.Mouse & GP2X_MOUSE_BTN_RELEASE){
        if (c.Mouse & GP2X_MOUSE_GST_LEFT) new_pad = GP2X_CTRL_LEFT;
        else
        if (c.Mouse & GP2X_MOUSE_GST_RIGHT) new_pad = GP2X_CTRL_RIGHT;
      }
    }
    else
# endif
...
static char user_filedir_92[PSP_FMGR_MAX_NAME];
...
psp_fmgr_menu(int format)
{
  ...
  if (first) {
    first = 0;
    strcpy(user_filedir_92, TI92.ti92_install_dir);
    strcat(user_filedir_92, "/92/");
    strcpy(user_filedir_kbd, TI92.ti92_install_dir);
    strcat(user_filedir_kbd, "/kbd/");
  }

  if (format == FMGR_FORMAT_KBD) user_filedir = user_filedir_kbd;
  else                           user_filedir = user_filedir_92;
  ...
  if (psp_file_request(user_filename, user_filedir)) {
    ...
    else 
    {
      ...
      if (file_format == FMGR_FORMAT_92) /* 92 */ error = ti92_load_prog_file(user_filename);
      else
