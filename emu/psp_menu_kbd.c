...
# endif
    ...
    if (menu_id == MENU_KBD_SKIN) {
      ...
      scan = strchr(buffer, '/');
      if (scan) *scan = 0;
      psp_sdl_back_print(80, y, buffer, color);
    } else
        ...
        (menu_id <= MENU_KBD_JOY_RIGHT)) 
    {
      ...
      if ((msx_key >= 0) && (msx_key < TI92_LAST_BUTTON)) {
        strcpy(buffer, ti92_buttons_info[msx_key].name);
      } else 
...
psp_keyboard_menu_reset_kbd(void)
{
  ...
  psp_sdl_back_print(180, 80, "Reset Keyboard !", 
...
psp_keyboard_menu_load()
{
  ...
  {
    psp_display_screen_kbd_menu();
    psp_sdl_back_print(180, 80, "File loaded !", 
                       PSP_MENU_NOTE_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
  ...
  {
    psp_display_screen_kbd_menu();
    psp_sdl_back_print(180, 80, "Can't load file !", 
                       PSP_MENU_WARNING_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
...
psp_keyboard_menu_mapping(int kbd_id, int step)
{
  ...
  if (ploc_kbd_mapping[kbd_id] <  -3) ploc_kbd_mapping[kbd_id] = TI92_LAST_BUTTON-1;
  else
  if (ploc_kbd_mapping[kbd_id] >= TI92_LAST_BUTTON) ploc_kbd_mapping[kbd_id] = -3;
}
...
psp_keyboard_menu_save()
{
  ...
  {
    psp_display_screen_kbd_menu();
    psp_sdl_back_print(180, 80, "File saved !", 
                       PSP_MENU_NOTE_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
  ...
  {
    psp_display_screen_kbd_menu();
    psp_sdl_back_print(180, 80, "Can't save file !", 
                       PSP_MENU_WARNING_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
...
# if defined(WIZ_MODE)
    int chk_menu_id = -1;
    for (chk_menu_id = 0; chk_menu_id < MAX_MENU_KBD_ITEM; chk_menu_id++) {
...
# endif
    ...
    if (danzeff_mode) {
      ...
      if (danzeff_key > DANZEFF_START) {
        msx_key = ti92_get_key_from_ascii(danzeff_key);
