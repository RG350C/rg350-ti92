...
# if defined(WIZ_MODE)
    if (c.My > (GP2X_SDL_SCREEN_HEIGHT - 20)) {
      if (c.Mouse & GP2X_MOUSE_BTN_PRESS) new_pad = GP2X_CTRL_DOWN;
    } else
    if (c.My < 20) {
      if (c.Mouse & GP2X_MOUSE_BTN_PRESS) new_pad = GP2X_CTRL_UP;
    } else
    if (c.Mouse & (GP2X_MOUSE_GST_LEFT|GP2X_MOUSE_GST_UP)) new_pad = GP2X_CTRL_LEFT;
    else
    if (c.Mouse & (GP2X_MOUSE_GST_RIGHT|GP2X_MOUSE_GST_DOWN)) new_pad = GP2X_CTRL_RIGHT;
# endif
