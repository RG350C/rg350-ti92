...
void
psp_editor_goto_mouse(gp2xCtrlData* c)
{
  g_text->cursor_moving = 1;

  int real_y_min = (GP2X_SDL_SCREEN_HEIGHT - (PSPWRITE.screen_h * psp_font_height)) / 2;
  int real_x_min = (GP2X_SDL_SCREEN_WIDTH  - (PSPWRITE.screen_w * psp_font_width)) / 2;

  int c_real_x = c->Mx;
  int c_real_y = c->My;
  int real_x_max = real_x_min + (PSPWRITE.screen_w * psp_font_width);
  int real_y_max = real_y_max + (PSPWRITE.screen_h * psp_font_height);
  if ((c_real_x > real_x_min) && (c_real_x < real_x_max) &&
      (c_real_y > real_y_min) && (c_real_y < real_y_max)) {
    int c_x = ( c_real_x  - real_x_min ) / psp_font_width;
    int c_y = ( c_real_y  - real_y_min ) / psp_font_height;
    g_text->curr_col  = g_text->left_col + c_x;
    g_text->curr_line = g_text->top_line + c_y;
...
# if defined(WIZ_MODE)
    if ( c.Mouse && ((! danzeff_mode) || (! danzeff_isMouseInside( &c )))) {

      if (c.Mouse & GP2X_MOUSE_GST_UP) psp_editor_goto_page_up();
      else
      if (c.Mouse & GP2X_MOUSE_GST_DOWN) psp_editor_goto_page_down();
      else
      if (c.Mouse & GP2X_MOUSE_GST_LEFT) psp_editor_goto_col_begin();
      else
      if (c.Mouse & GP2X_MOUSE_GST_RIGHT) psp_editor_goto_col_end();
      else
      if (c.Mouse & GP2X_MOUSE_CLICK) psp_editor_goto_mouse( &c );
